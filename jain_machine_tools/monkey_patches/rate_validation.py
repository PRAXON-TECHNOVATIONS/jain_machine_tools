...
def _patched_validate_rate_with_reference_doc(self, ref_details):
	"""
	Allow lower rates on Purchase Invoice / Sales Invoice against their source docs.
	Only block when the current rate is greater than the reference rate.
	"""
	if self.doctype not in ("Purchase Invoice", "Sales Invoice"):
	...
	for ref_dt, ref_dn_field, ref_link_field in ref_details:
		...
		for d in self.get("items"):
			...
			if current_rate - ref_rate >= 0.01:
				stop_actions.append(
					_(
						"Row #{0}: Rate cannot be greater than {1}: {2} ({3} > {4})"
